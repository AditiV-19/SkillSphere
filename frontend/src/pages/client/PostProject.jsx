...
import { Briefcase, DollarSign, MapPin, Plus, Trash2, Send, AlertCircle, FileText } from "lucide-react";
...
export default function PostProject() {
        ...
        <div className="p-8 min-h-screen bg-slate-50 flex items-center justify-center">
          <div className="bg-white border border-rose-200 rounded-2xl p-6 text-center shadow-sm max-w-md">
            ...
            <p className="text-slate-500 text-sm mt-1">
              Only registered clients can post projects to the marketplace ecosystem.
            </p>
          </div>
        </div>
      </DashboardLayout>
  ...
  const handleSubmit = async (e) => {
    ...
    // Dynamic validations matching backend boundaries
    ...
    // Process comma-separated text into a clean string array
    ...
    const payload = {
      ...
      state: remote ? undefined : stateName,
      country: remote ? undefined : country,
      latitude: remote ? undefined : latitude,
      longitude: remote ? undefined : longitude
    };
    ...
    try {
      ...
      setError(err.response?.data?.message || "An error occurred while compiling your project submission.");
      console.error(err);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-200/60">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Post a Project</h1>
            ...
            <p className="text-slate-500 mt-1.5 text-sm">
              Deploy smart milestone assignments to hire verified professionals across the local network.
            </p>
          </div>
        </div>
          ...
          <div className="col-span-12 lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
              <div className="flex items-center gap-2 pb-3 border-b border-slate-100 text-slate-800">
                ...
                <h2 className="font-bold text-base tracking-tight">Project Overview</h2>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2 text-slate-800">
                  ...
                  <h2 className="font-bold text-base tracking-tight">Milestone Payments (Escrow)</h2>
                </div>
                <span className="text-xs text-slate-400 font-medium">Optional Setup</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-slate-50/60 p-4 rounded-xl border border-slate-100">
                <div className="md:col-span-7">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Deliverable Name</label>
                  <input
                    type="text"
                ...
                <div className="md:col-span-3">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Fund Allocation ($)</label>
                  <input
                    type="number"
              ...
              {milestones.length > 0 && (
                <div className="space-y-2">
                  {milestones.map((ms, index) => (
                    <div key={index} className="flex items-center justify-between p-3 rounded-xl bg-blue-50/40 border border-blue-100 text-sm">
                      <div className="flex items-center gap-2 font-medium text-slate-700">
                        <span className="text-blue-600 font-bold bg-blue-50 px-2 py-0.5 rounded text-xs">M{index + 1}</span>
                        <span>{ms.name}</span>
                      </div>
                      <div className="flex items-center gap-3">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6 lg:sticky lg:top-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
              <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100 text-slate-800">
                ...
                <h2 className="font-bold text-base tracking-tight">Budget Scale</h2>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Min Budget ($)</label>
                    <input
                      required
                      type="number"
                      value={minBudget}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Max Budget ($)</label>
                    <input
                      required
                      type="number"
                      value={maxBudget}
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
              <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100 text-slate-800">
                ...
                <h2 className="font-bold text-base tracking-tight">Work Environment</h2>
              </div>

              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <span className="text-xs font-bold text-slate-600 uppercase">Remote Configuration</span>
                  <input
                    type="checkbox"
                    checked={remote}
